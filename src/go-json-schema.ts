/**
 * JSON-schema normalization for tool parameters sent to /alpha/generate.
 *
 * Ported from patlux/pi-commandcode-provider v0.7.1 (commit 6fd0ac7)
 * src/json-schema.ts. Pure functions, no secrets involved.
 */
...
function copySchemaObject(
	...
	for (const [key, value] of Object.entries(source)) {
		...
		if (key === "required") {
			...
			out[key] = Array.isArray(value) && key === "items" ? schemaArray(value, seen) : schemaValue(value, seen);
		} else {
			out[key] = value;
		}
	}
...
function convertLegacySchema(
	...
	if (kind === "union" || kind === "intersect" || kind === "intersection") {
		const variants = legacyVariants(source)
			...
			.filter(
				(variant) =>
					isSchemaValue(variant) && (typeof variant === "boolean" || Object.keys(variant).length > 0),
			);
	...
	const scalarType =
		kind === "string" || kind === "number" || kind === "boolean" || kind === "integer" || kind === "null"
			? kind
			: undefined;
...
/**
 * Normalize nullable type arrays for Gemini's generate transport.
	...
	for (const [key, value] of Object.entries(schema)) {
		...
		const converted =
			SCHEMA_MAP_FIELDS.has(key) && isRecord(value)
				? Object.fromEntries(Object.entries(value).map(([name, child]) => [name, geminiSafeJsonSchema(child)]))
