 ...
 * Secret redaction and context-overflow normalization for the Go transport.
 *
 * Ported from patlux/pi-commandcode-provider v0.7.1 (commit 6fd0ac7)
 * src/overflow.ts. Never log keys, tokens, or authorization headers: every
 * error message leaving this module goes through redactCommandCodeErrorText.
 */
...
export function normalizeCommandCodeErrorMessage(errorMessage: string | undefined): string | undefined {
	...
	if (HTTP_RATE_LIMIT_STATUS_PATTERNS.some((pattern) => pattern.test(errorMessage))) return undefined;
	if (!COMMAND_CODE_OVERFLOW_PATTERNS.some((pattern) => pattern.test(errorMessage))) return undefined;
